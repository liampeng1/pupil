...
class Screen_Marker_Calibration(Calibration_Plugin):
    ...
    def is_fixation(self):
        fixation_list = []
        pupil_pts = [p for p in pupil_list[-5:] if p['id'] == 0]
        for x in range(2,size(self.pupil_list)-2):
            max_x = pupil_pts[x]['norm_pos_x']
            min_x = pupil_pts[x]['norm_pos_x']
            max_y = pupil_pts[x]['norm_pos_y']
            min_y = pupil_pts[x]['norm_pos_y']
            for i in range(-2,2):
                pos_x = pupil_pts[x]['norm_pos_x']
                pos_y = pupil_pts[x]['norm_pos_y']
                if pos_x > max_x:
                    max_x = pos_x
                if pos_x < min_x:
                    min_x = pos_x
                ...
                if pos_y < min_y:
                    min_y = pos_y
            if max_x - min_x < 0.003 && max_y - min_y < 0.003:
                fixation_list.append(pupil_pts[x])
